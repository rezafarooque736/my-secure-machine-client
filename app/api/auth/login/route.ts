...
// 2. Checks admin permissions via /api/session/data/{ds}/users/{u}/permissions
// 3. Creates a UserSession record in Prisma
// 4. Writes an ActivityLog AUTH SUCCESS entry
// 5. Returns { authToken, username, dataSource, availableDataSources, role, sessionId }
// ─────────────────────────────────────────────────────────────────────────────
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── Step 2: Determine admin role ───────────────────────────────────────
    let role: "admin" | "user" = "user";

    try {
      const permRes = await fetch(
        `${GUACAMOLE_URL}/api/session/data/${dataSource}/users/${guacUsername}/permissions?token=${authToken}`,
        { headers: { "Content-Type": "application/json" } }
      );

      if (permRes.ok) {
        const permData = await permRes.json();
        if (permData?.systemPermissions?.includes("ADMINISTER")) {
          role = "admin";
        }
      }
    } catch (permErr) {
      // Non-fatal — default to "user" role
      console.warn("[AUTH] Could not fetch permissions, defaulting to user role");
    }
    ...
    console.log(
      `[AUTH] Session created — id: ${session.id} | user: ${guacUsername} | role: ${role}`
    );
