...
//   username   – optional: filter by a specific user (admin use)
//
    ...
    await Promise.all(
      connectionIds.map(async (connId) => {
        try {
          ...
          const history: any[] = Array.isArray(histRes.data)
            ? histRes.data
            : Object.values(histRes.data);

          history.forEach((record: any) => {
            // Optional username filter (for non-admin self-view)
            if (
              usernameFilter &&
              record.username !== usernameFilter
            ) {
              return;
            }
            ...
            allSessions.push({
              ...
              historyEntryIdentifier:
                record.identifier ??
                record.historyEntryIdentifier ??
                `${connId}-${record.startDate}`,
              connectionId: connId,
              connectionName:
                connectionsMap[connId]?.name ?? `Connection ${connId}`,
              ...
              // Protocol
              protocol: (
                connectionsMap[connId]?.protocol ?? "unknown"
              ).toUpperCase(),
